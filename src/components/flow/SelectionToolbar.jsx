...
export default function SelectionToolbar({ selectedIds }) {
  const { rfNodes, workflowPhase, extractAssets, generateFromSelection, isGenerating } = useProject()
  ...
  return (
    <Panel position="bottom-center" className="mb-6">
      <div className="flex items-center gap-2 bg-canvas-900/95 backdrop-blur border border-line rounded-xl px-3 py-2 shadow-2xl shadow-black/40">
        <span className="text-[14px] text-neutral-500 mr-1">
        ...
        {canExtract && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-violet-600 hover:bg-violet-500 text-white text-[15px] font-medium transition-all active:scale-95 shadow-md shadow-violet-900/40"
          >
            <Sparkles size={12} />
            ✨ 提炼核心资产
          </button>
        ...
        {canGenerate && (
          <button
            ...
            className={cn(
              'flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[15px] font-medium transition-all active:scale-95 shadow-md',
              isGenerating
