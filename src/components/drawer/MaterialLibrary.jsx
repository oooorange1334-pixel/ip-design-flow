    ...
    <div
      ...
      className={cn(
        ...
        isDragging
          ...
          : 'border-line hover:border-neutral-600 hover:shadow-md',
      )}
    >
      {/* 图片 */}
      ...
      <div className="px-2 py-1.5 bg-canvas-900/90">
        <p className="text-[14px] font-medium text-neutral-300 truncate">{item.label}</p>
        <div className="flex flex-wrap gap-0.5 mt-0.5">
          {item.tags?.slice(0, 2).map(tag => (
            <span key={tag} className="text-[14px] text-neutral-600 bg-neutral-800 px-1 rounded">
              {tag}
            </span>
          ))}
        </div>
      ...
      {!isDragging && (
        <div className="absolute inset-x-0 bottom-0 h-6 bg-gradient-to-t from-accent/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center pb-1 pointer-events-none">
          <span className="text-[14px] text-accent/80">拖拽到画布</span>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="mb-3">
      ...
      >
        ...
        <span className="text-[15px] text-neutral-700 ml-auto">{items.length}</span>
        <span className={cn('text-[15px] text-neutral-700 transition-transform', collapsed ? '' : 'rotate-90')}>›</span>
      </button>
...
export default function MaterialLibrary() {
  const { materialLibrary, workflowPhase, lockedElements } = useProject()
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-line shrink-0">
        <div className="flex items-center gap-2 mb-1">
          ...
          <p className="text-[14px] font-semibold text-neutral-100">素材库</p>
          {lockedElements.length > 0 && (
            <span className="ml-auto text-[15px] text-locked font-mono">{lockedElements.length} 已锁定</span>
          )}
        </div>
        <p className="text-[14px] text-neutral-600">将卡片拖入画布，组装参数后生成</p>
      </div>
      ...
      {isExtracting && (
        <div className="flex-1 flex flex-col items-center justify-center gap-4 px-4">
          ...
          <div className="text-center">
            ...
            <p className="text-[14px] text-neutral-600 mt-1">提炼 Form · CMF · Motif</p>
          </div>
          <div className="flex gap-1">
            {['形体', '材质', '符号'].map((label, i) => (
              <span
                key={label}
                className="text-[15px] px-2 py-0.5 rounded-full border border-line text-neutral-600"
      ...
      {!isExtracting && (
        <div className="flex-1 overflow-y-auto px-3 pt-3">
          ...
          {/* 使用提示 */}
          <div className="mt-2 mb-4 rounded-lg border border-line bg-canvas-800/30 p-3">
